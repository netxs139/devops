...
本脚本已重构为支持 CLI Phase 2 (Deep Integration) 调用。
"""
...
CSV_FILE = Path(__file__).parent.parent / "docs" / "assets" / "sample_data" / "organizations.csv"


def execute_command(session: Session, **kwargs) -> bool:
    """[Phase 2 改造] 从 CSV 文件加载组织架构。"""
    ...
    try:
        ...
        # Phase 2 模式下不在此处 commit，由 cli.py 统一管理事务
        session.flush()
