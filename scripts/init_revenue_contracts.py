...
重构说明：已移除硬编�?SAMPLE_CONTRACT，改为从 docs/assets/sample_data/revenue_contracts.csv 加载�?
"""
...
def init_revenue_contracts():
    ...
    try:
        if not os.path.exists(CSV_FILE):
            logger.warning(f"跳过收入合同初始化：未找�?{CSV_FILE}")
            return
        ...
        with open(CSV_FILE, encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                if not contract:
                    ...
                    logger.info(f"录入新合�? {contract_no}")
                ...
                session.add(node)
                logger.info(f"  -> 自动生成首付款阶�? 金额 {billing_amount}")
