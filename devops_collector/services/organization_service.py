...
class OrganizationService:
    ...
    def sync_from_csv(self, csv_path, progress_callback=None):
        """从 CSV 同步组织架构主数据。"""
        import csv
        ...
        for row in rows:
            self.upsert_organization(
                org_code=row.get("部门ID", "").strip(),
                org_name=row.get("部门名称", "").strip(),
                org_level=int(row.get("层级", 1)),
                parent_org_code=row.get("上级ID", "").strip() or None,
                manager_raw_id=row.get("负责人ID", "").strip() or None,
                source="csv_init",
            )
