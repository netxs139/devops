...
## name: plugin-init description: Workflow for plugin-init
...
1. 在 `devops_collector/core/worker_factory.py` 中注册 `worker` 任务。
1. 执行 `just migrations` 生成 Alembic 脚本。
...
1. 运行基础 mock 测试：`pytest tests/plugins/[prefix]_[component]/`。
