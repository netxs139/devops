...
## name: evolve-skill description: Workflow for evolve-skill
...
1. **示例**：如果教训是“JSONB 引号陷阱”，则创建一个包含 `(raw->>'f')::boolean` 的 SQL 文件。

## Phase 3: 注入评估集 (Eval Ingestion)

1. **读取 Skill Evals**：打开目标 Skill 的 `evals/evals.json`。
1. **新增用例**：向数组中插入一个新的评估用例，明确指出：`"input": "/tmp/evolve/..."`, `"expected_focus": "捕获该特定的 [教训描述] 风险"`。

## Phase 4: 技能指令突变 (Instruction Mutation) [CORE]

1. **修改 SKILL.md**：打开对应 Skill 的指令主文件。
1. **注入防御逻辑**：在 `## 审计清单` 或 `## 执行规程` 中增加一条精准的防御规则。
1. **格式要求**：使用 `[Ref LL#{ID}]` 标记来源，以便后续追溯。
...
1. **公告**：更新 `progress.txt` 的“最近完成”：“技能演进完成：[Skill Name] 现已具备防御 [LL描述] 的能力。”
