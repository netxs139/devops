...
## name: session-handover description: Workflow for session-handover
...
```bash
# 读取最近 15 条 Lessons Learned
tail -15 docs/history/lessons-learned.log
```

### 3. 识别未完成工作与中断的临时任务 (15s)

- 检查 `progress.txt` 的 [进行中] 区域
- 检查 `git stash list` 是否有暂存的工作
- 检查是否有未合并的 Feature Branch
- 🚨 **重点扫描** `git status -u` 输出中是否包含未跟踪的临时脚本（特别是 `.agent/scratch/` 目录下，或 `fix_*.py` 等）。若存在，判定为“上次发生了异常中断的排错任务”。此时**必须主动挂起**，提示用户并询问是否需要整理归档这些临时产物。
...
沉淀时，在 `docs/history/lessons-learned.log` 末尾追加一行，格式与字段为：
...
### 4. 环境卫生与清场取证 (Hygiene & Clearance Evidence) [MANDATORY]
...
# 2. 取证核验：检查是否有未记录的遗留文件 (执行 中断恢复嗅探律)
git status -u
```
...
- [ ] **进度状态更新 (双向同步)**：`progress.txt` 已更新。
  ...
  - [ ] 2. 🚨 **[强制归档检查]**: 确认 `progress.txt` 中的已完成条目**绝对不超过 5 条**。若溢出，必须立刻执行裁剪，并将其安全追加至 `CHANGELOG.md` 的最新版本或 `[Unreleased]` 节段。
...
1. **检查一致性**：`progress.txt` 中记录的状态是否与物理事实一致。
1. **若发现不一致**：以物理事实为准，修正 `progress.txt`。
