...
description: |
  专门审查代码库中涉及数据库模型关联（ORM）、表架构变更、主数据同步与数据字典设计的高级代码评审代理。当用户提到“架构变更”、“建模”、“数据字典变更”、“增加数据库字段”、同步死锁、循环关联或 SQLAlchemy 逻辑审查时必须触发。核心核查点：1) 两阶段对齐协议（处理循环外键）；2) 审计日志属性过滤（防 N+1 序列化）；3) UUID 强类型与方言感知（on_conflict fallback）；4) .is_(None) 谓词规则与 FK 显式索引。
---
