...
## name: deploy description: Workflow for deploy
...
1. **数据库静默备份**：
   ```bash
   just db-backup  # 备份 devops_db 至 artifacts/backups/
   ```
1. **环境变量校验**：检查生产环境 `.env` 的敏感密钥是否存在缺失（对比 `.env.example`）。
