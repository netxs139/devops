...
## name: code-review description: Workflow for code-review
...
扫描 `docs/history/lessons-learned.log`，自动关联与本次变更相关的历史教训：

1. **按 Domain 匹配**：识别本次变更涉及的领域（如 ZenTao、DB、MQ、Auth 等），检索历史记录中同域的 Universal Rule。
1. **交叉验证**：检查本次代码变更是否违反了已有的 Universal Rule（如：是否在循环中调用 `create_engine`？是否直接存储了 `dict` 对象？）。
1. **输出**：在审查报告中增加 `⚠️ 关联教训` 部分，列出相关的防御规则。
...
> **动作**：AI 在审查阶段必须显式重读并对齐 [`contexts.md#11.1`](contexts.md#L605) (业务域前缀注册表)。
...
- **验证通过证据**：[提供 just verify 的全绿输出，且 Coverage >= 80%]
- **结论**：[允许合并 / 需先重构]
