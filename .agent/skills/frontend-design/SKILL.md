...
## name: frontend-design description: 创建独特的、生产级别的高质量前端界面。当用户要求构建网页组件、页面、作品、海报或应用程序时使用此技能（例如网站、落地页、仪表盘、React 组件、HTML/CSS 布局，或对任何网页 UI 进行样式美化）。生成富有创意、精致的代码和 UI 设计，避免千篇一律的 AI 审美。 license: 完整条款见 LICENSE.txt
...
1. **管理门户 (DevOps Portal)**: 运行在 `devops_portal/static/` 目录下，基于 **Vanilla JS**。
...
1. 若为新页面，**必须先在该表中注册 Index + Page Module + Domain**，再编写任何 HTML。
1. 示例检查指令：`grep -n "traceability" contexts.md` — 确认未被已有条目占用。
...
1. 禁止在 JS 中用字面量硬编码 API 响应结构，必须有 Python 侧的 Schema 作为"物理契约"。
