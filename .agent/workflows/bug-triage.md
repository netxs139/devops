...
## Step 0: 历史教训前置审计 (Mandatory Pre-audit)

在开始任何排障动作（包括编写复现用例）前，AI **必须**首先执行：
1. **全量扫描** `docs/lessons-learned.log`。
2. **关键词匹配**：根据故障现象（如 `401`, `AttributeError`, `Missing Field`）进行历史经验检索。
3. **输出声明**：在回复中显式包含 `[Pre-audit]: 扫描 lessons-learned.log 发现/未发现相关记录 [编号/规则名称]。`
