...
## Step 1.2: 专家技能强制路由 (Expert Skill Routing) [MANDATORY]

根据任务领域，Agent **必须首先激活** 对应的专家 Skill 进行前置评估或审计关联：

| 涉及领域 | 对应专家 Skill | 强制动作 |
| :--- | :--- | :--- |
| **dbt 模型 / SQL 逻辑** | `dbt-pipeline-auditor` | 扫描 Staging/Int 模型，检查 JSONB 陷阱与 SCD2 历史完整性。 |
| **DB Schema / MDM 建模**| `mdm-integrity-arbiter` | 审查 ORM 关联、UUID 强类型与两阶段对齐协议。 |
| **韧性验证 / 高危变更** | `chaos-sentinel` | 注入仿真故障验证防御逻辑，执行故障注入审计。 |

**验证要求**：在 Kickoff 汇报中必须声明：`[Expert Skill]: [Skill Name] 已挂载审计。`
...
[Kickoff Complete] Level: Lx | ETA: Xh | Branch: feat/xxx | Tech Check: OK | Expert Skill: [Skill Name] | Risks: N identified

> [!IMPORTANT]
> **根据 `AGENTS.md` Rule 9 守护：方案已就绪，是否授权开始物理编码？ (请用户回复“开始/OK”以解锁代码修改权限)**
