...
def gitlab_issue_payload(event_type="Issue Hook", labels=None, old_labels=None, iid=42, action="open", project_id=99):
...
def test_gitlab_webhook_test_case_label(webhook_client):
    ...
    response = webhook_client.post("/webhooks/gitlab", json=data["payload"], headers={"X-Gitlab-Event": "Issue Hook"})
...
def test_gitlab_webhook_requirement_review_state_change(webhook_client):
    ...
    with (
        patch("devops_portal.routers.webhook_router.get_requirement_author", return_value=None),
        patch("devops_portal.routers.webhook_router.asyncio.create_task"),
    ):
        response = webhook_client.post("/webhooks/gitlab", json=payload, headers={"X-Gitlab-Event": "Issue Hook"})
...
def test_gitlab_webhook_pipeline_hook_updates_state(webhook_client):
    ...
    response = webhook_client.post("/webhooks/gitlab", json=payload, headers={"X-Gitlab-Event": "Pipeline Hook"})
...
def test_gitlab_webhook_unknown_event(webhook_client):
    ...
    response = webhook_client.post("/webhooks/gitlab", json={"foo": "bar"}, headers={"X-Gitlab-Event": "Push Hook"})
    assert response.status_code == 200
    assert response.json()["status"] == "accepted"
