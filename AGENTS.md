...
| **`/evolve-skill`** | 产生新教训 | 将 `lessons-learned` 转化为自动化审计规则 (Arbiter)；**完工后必须在来源条目末尾追加 `[Codified into Skill: <skill-name> §<N> at <date>]`** 回标，防止重复固化 |
...
1. **交接审计自进化**: [L2-L4] 离场 DoD 强制执行 `/evolve-skill`；严禁在未持久化日志时宣告完工。完工 DoD 包含两步物理检查点：① Skill 文件写入；② 在 `lessons-learned.log` 来源条目末尾追加 `[Codified into Skill: <skill-name> §<N> at <date>]` 回标。
...
*Generated by Antigravity-Core (2026-06-09T08:42:00+08:00)*
