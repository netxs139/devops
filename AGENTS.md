...
1. **性能分析与取证授权律 (The Law of Performance Profiling) [NEW/MANDATORY]**:
   ...
   - **核心约束**: 对于涉及大规模数据同步、核心模型重构或 Dashboard 高维运算的 [L3/L4] 任务，Agent **被授权且强制要求** 在方案设计阶段执行「物理性能取证」。当涉及 L3/L4 级重构时，Agent 有权主动运行性能采样工具并基于真实 Trace 数据提出建议。
   - **执行要求**: 必须主动使用 `run_command` 执行 `just profile-db` (执行 `EXPLAIN ANALYZE`) 或 `just profile-code` (运行 `py-spy`) 等分析工具。
...
1. **交付报告物理对齐律 (Evidence-as-Delivery) [NEW/MANDATORY]**:
...
1. **意图锚点与 ADR 查阅律 (The ADR Intent Anchor Rule) [NEW/MANDATORY]**:

   - **核心约束**: `progress.txt` 记录了“做什么”，而 `docs/adr/` 记录了“为什么不那样做”。在进行任何代码审查或提出架构层面的“优化”建议前，Agent **必须优先查阅** `docs/adr/` 目录下的相关记录。
   - **防线逻辑**: 遇到看似“笨重”、“冗余”或“非标准”的设计（例如放弃某优美语法的兼容性妥协）时，严禁以“代码整洁之道”等通用标准盲目推翻。必须先核对 ADR 以防止破坏既定的物理边界，减少沟通损耗。
   - **强制动作**: 每当业务开发中产生新的技术妥协、避坑决策或特殊的边界条件选取，必须在 `docs/adr/` 目录下生成极简架构决策记录，作为永久的意图锚点。
