...
| **新坑位发现** | `docs/history/lessons-learned.log` | `skill-creator` | 每当产生 Blocker 级教训，强制触发技能重构，将坑位转化为自动化用例。 |
...
1. **环境自愈清场**：离场前强制执行 `make clean`。**严禁**在根目录残留任何调试脚本、临时日志或导出文件。

1. **安全审计律 (The Law of Security)**：涉及 core/mdm 核心表变更或引入新依赖的 [L3/L4] 任务，**必须执行** `make security-audit`。

   - **标准**：必须通过 Bandit (代码逻辑安全)、detect-secrets (无机密泄露) 与 Safety (无高危依赖)。
   - **操作要求**：必须在交付报告中包含安全审计摘要。

1. **绝对取证原则**：进度汇报必须包含控制台输出碎片。**严禁**出现“逻辑预测认为代码正确”的幻觉。
...
1. **文档完整性守则 (The Law of Document Integrity) [MANDATORY]**:
...
1. **中断恢复嗅探律 (The Interruption Recovery Rule) [NEW]**:
   ...
   - **动作映射**: 如果发现未跟踪的临时文件（特别是 `.agent/scratch/` 下）或未记录在 `progress.txt` 中的改动，AI 必须判定为“前次会话异常中断”。此时必须主动暂停，向用户汇报发现的“进度碎片”，并询问是否需要执行整合并入 `progress.txt`。
