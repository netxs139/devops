...
with m1:
    ...
    st.markdown(
        f'<div class="metric-card"><p class="metric-label">发布频率 (Release)</p><p class="metric-value">{val:.1f}</p><small>次/月</small></div>',
        unsafe_allow_html=True,
    )
with m2:
    val = latest_data["lead_time_hours"].mean()
    st.markdown(
        f'<div class="metric-card"><p class="metric-label">前置时长 (Lead Time)</p><p class="metric-value">{val:.1f}h</p><small>平均工作耗时</small></div>',
        unsafe_allow_html=True,
    )
with m3:
    val = latest_data["change_failure_rate_pct"].mean()
    st.markdown(
        f'<div class="metric-card"><p class="metric-label">变更失败率 (CFR)</p><p class="metric-value" style="color:#ef4444">{val:.1f}%</p><small>生产事故率</small></div>',
        unsafe_allow_html=True,
    )
with m4:
    val = latest_data["mttr_hours"].mean()
    st.markdown(
        f'<div class="metric-card"><p class="metric-label">事故恢复 (MTTR)</p><p class="metric-value">{val:.1f}h</p><small>P1 事故平均修复</small></div>',
        unsafe_allow_html=True,
    )
...
with c1:
    fig_line = px.line(filtered_df, x="audit_month", y="deployment_frequency", color="product_name", title="发布频率趋势 (Release Count)", markers=True)
    st.plotly_chart(fig_line, use_container_width=True)

with c2:
    fig_bar = px.bar(filtered_df, x="audit_month", y="lead_time_hours", color="product_name", title="交付时长波动 (Lead Time Hours)", barmode="group")
