...
- **计算逻辑**:
  - 基于 ELOC，但增加权重：
  - **老代码修改 (Legacy Refactor)**: 权重 1.5x (修改 >6个月/180天前的代码) — ✅ `eloc.py` 文件级判定
  - **广度 (Breadth)**: 权重 `log(涉及文件数 + 1)` (跨模块修改更复杂) — ✅ `commit_mixin.py` Commit 聚合级应用
  - **复杂度 (Complexity)**: 权重 `f(圈复杂度变化)` (如果可用) — ⏸️ 未实现，待 SonarQube 数据对接
...
## 4. 实现状态 (Implementation Status)

> Last Updated: 2026-04-29

| 计划项 | 状态 | 实现位置 |
| :--- | :--- | :--- |
| Schema 迁移 (CommitMetrics 扩展) | ✅ 完成 | `base_models.py` — `eloc_score`, `impact_score`, `churn_lines`, `file_count` 等字段已就位 |
| ELOCAnalyzer (ELOC + Impact) | ✅ 完成 | `core/analytics/eloc.py` — FileWeight, LineWeight, BehaviorWeight, TestMultiplier, LegacyFactor |
| BreadthFactor 聚合 | ✅ 完成 | `plugins/gitlab/mixins/commit_mixin.py` — `log(file_count + 1)` 在 Commit 聚合级应用 |
| Churn 检测 (21天阈值) | ✅ 完成 | `commit_mixin.py` (文件级判定) + `eloc.py` (行级计数) |
| Active Days 统计 | ✅ 完成 | `fct_developer_activity_profile.sql` — `count(distinct metric_date)` (dbt Marts 层) |
| DailyAggregator (每日快照) | ✅ 等价完成 | `dws_developer_metrics_daily.sql` 替代了 Python 侧的 DailyDevStats 表设计 |
| ComplexityFactor (圈复杂度) | ⏸️ 待定 | 依赖 SonarQube 的函数级复杂度数据对接 |
| Sherpa Score (协作贡献) | ⏸️ 待定 | 需要 MR Review Comment 的细粒度采集 |
| 可视化升级 | ⏸️ 待定 | Streamlit Value Leaderboard 尚未增加 Quality/Collaboration 选项卡 |
