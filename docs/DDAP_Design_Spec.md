...
> **版本**: v3.0 | **日期**: 2026-07-06 | **作者**: AI Agent
> **代号**: DDAP
>
...
## 3. 被废弃的旧版数据模型 (Deprecated)

为了平滑过渡到新架构，以下原存在于 `devops_collector/models/base_models.py` 中的实体已被废弃，将在代码中被物理移除：

| 旧版表名/实体 | 替代方案 / 移交模块 |
| --------------------------- | ------------------------------------------- |
| `mdm_identities` (User) | 移交至 `identity_module` (`IdentityUser`) |
| `mdm_organizations` (Org) | 移交至 `identity_module` (`IdentityDepartment`)|
| `SysRole`, `SysMenu` | 移交至 `identity_module` (RBAC 系统) |
| `IdentityMapping` | 移交至 `identity_module` OIDC 机制 |
