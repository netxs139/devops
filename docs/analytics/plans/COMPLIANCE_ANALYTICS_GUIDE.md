...
1. **Dev + Admin**: 开发者同时拥有超级管理员权限
1. **关键岗位兼任**: 同一人同时担任 Dev + DBA + Ops
...
1. **标记离职员工**: 及时更新 `users.state` 为 `inactive`
1. **记录部署者信息**: 确保 `deployments` 表包含部署者 ID
1. **集成许可证 API**: 对接 SPDX 或 ClearlyDefined API 获取准确的许可证信息
