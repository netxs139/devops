...
- **核心逻辑**:
  ...
  1. 聚合 `commits` 数据，统计过去 90 天内各项目的活跃贡献者和提交量。
  1. 计算每个项目在所属业务线内的资源占比。
- **度量指标**:
...
- **核心逻辑**:
  ...
  1. 聚合计算 DORA 核心指标：发布频率、变更前置时间、变更失败率。
  1. 使用 `RANK()` 函数生成“速度榜”和“质量榜”。
- **度量指标**:
...
- **核心逻辑**:
  ...
  1. 关联 SonarQube (质量门禁, Bug, 覆盖率)。
  1. 集成 Tags/Releases (交付版本)。
- **度量指标**:
...
- **核心逻辑**:
  ...
  1. **Y轴 (Health)**: 基于 Bug、漏洞和覆盖率计算健康得分。
  1. **分类**: 根据 50/60 分界线自动划分为四个象限。
- **度量指标**:
...
- **核心逻辑**:
  ...
  1. **安全**: 统计 SonarQube 中未修复的 `BLOCKER/CRITICAL` 漏洞及其平均积压时长。
- **度量指标**:
...
- **核心逻辑**:
  ...
  1. **复用 (Reuse)**: 统计 Star > 5 或 Fork > 2 的项目比例，视为“企业级公共资产”。
- **度量指标**:
...
- **核心逻辑**:
  ...
  1. **跨域碰撞**: 筛选供应项目所属部门 != 消费项目所属部门的记录。
  1. **热力计算**: 以供应部门为 X 轴，消费部门为 Y 轴，复用项目数为热力强度。
- **度量指标**:
...
- **核心逻辑**:
  1. **响应性 (SLA)**: 以 Bug MTTR 为基准。每超过 24h 扣 10 分（上限 40）。公式：$Penalty\_{SLA} = (MTTR / 24) * 10$。
  1. **争议度 (Controversy)**: 统计评论数 > 10 的 Issue 占比。反映方案冲突，扣分权重 1.5（上限 30）。
  1. **返工率 (Rework)**: 统计 Reopened 的任务占比。反映交付质量，扣分权重 3.0（上限 30）。
- **度量指标**:
  - **SSI 指数**: 0-100 分综合得分。
...
- **核心逻辑**:
  ...
  1. **协作熵 (Collab Entropy)**: 计算 MR 平均人工备注（Comment）数。代表单次变更触发的探讨密度。
  1. **评审乒乓 (Ping-Pong Index)**: 统计 MR 状态打回 (Reopened) 的平均轮次。过高 ( > 4) 代表协作低效。
...
- **核心逻辑**:
  ...
  1. **代码流转率 (Churn)**: 统计过去 90 天的提交频次，反映模块活跃度。
  1. **技术硬伤 (Risk)**: 提取 SonarQube 的圈复杂度与单测覆盖率。
...
- **核心逻辑**:
  ...
  1. **延期惯性 (Delay Inertia)**: 统计 Issue 变更 history 中 `due_date` 被延后的频率。
  1. **完成质量 (Completion Quality)**: 结合是否在迭代内完成。
...
- **核心逻辑**:
  ...
  1. **Output**: 聚合已合并的 MR (技术产出) 和已关闭的 Issue (业务产出)。
  1. **Efficiency**: 计算两者比率。
- **度量指标**:
...
- **核心逻辑**:
  ...
  1. **流程守护 (Process Score)**: 统计对 `gitlab_issue_events` 中标签 (Labels)、里程碑 (Milestones) 的维护频次，以及对 Jira `due_date` 的调整动作。识别主动优化流程、确保项目合规的"守护者"。
  1. **协作催化 (Help Score)**: 统计在 `notes` 表中的人工评论活跃度（排除系统消息）。识别积极响应、解决争议、解除他人阻塞的"催化剂"。
...
- **核心逻辑**:
  ...
  1. **交付漏斗 (Pipeline Pass Rate)**: 计算 `成功构建数 / 总构建数`，识别"试错型"项目（构建成功率低，反复重试）。
  1. **环境流转时延 (Env Dwell Time)**: 通过 `deployments` 表，关联 Staging 与 Production 环境的 `ref` (branch/tag)，计算同一制品在两个环境间的时间差。这反映了制品在 Staging 环境的"停留时长"。
  1. **构建发布比 (Build per Release)**: 计算 `总构建次数 / 生产发布次数`，识别构建浪费（如每次发布需要 15+ 次构建，说明流程不稳定）。
- **计算公式**:
  - $DwellTime = AVG(Production.created_at - Staging.created_at)$ (单位: 小时)
...
- **核心逻辑**:
  ...
  1. **依赖抓取**: 从 `traceability_links` 中提取 `blocks` 的跨部门阻塞关系。
  1. **脆弱性量化**: 统计各部门受外部影响的任务比例。
...
1. **`process_deviation.sql`**: 如果发现某部门的“游离代码率”异常高，PMO 应介入审计其合规性。
1. **`HR_ANALYTICS.sql` 中的 `view_hr_user_tech_stack`**: 可以上卷到部门层级，生成“部门技术栈大图”，辅助技术战略选型（例如：我们部门是不是 Python 太多了，Java 太少了？）。
...
1. **单价端 (Rate)**: 基于 `users.job_title_level`（职级）在 `labor_rate_configs` 表中查找对应的 `hourly_rate`（标准时薪）。
1. **归一化 (Normalization)**: 系统自动将不同系统的账号对齐到全局 User，确保工时能够精准挂接到人员及其单位成本。
...
1. **Python 引擎**: 编写 `scripts/wecom_risk_bot.py` 定时运行，识别异常并调用 Webhook。
