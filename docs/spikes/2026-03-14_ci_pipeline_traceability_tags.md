...
## status: Proposed date: 2026-03-14 author: AI Architect topic: CI/CD Pipeline Artifact Traceability Strategy (Jenkins & GitLab CI)
...
1. 将解析出来的 `commit_sha` 关联键更新入 `nexus_components` 模型中新扩充的字段。
1. 利用 dbt 生成终极事实表 `fct_lead_time_from_commit_to_nexus()`。
