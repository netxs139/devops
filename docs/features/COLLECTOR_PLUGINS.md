...
- **价值流分析 (VSM) [NEW]**:
  - **状态流转追踪**: 物理记录 MR 全生命周期状态变迁（Opened -> Closed -> Merged）。
  - **等待时长量化**: 自动识别从 Draft 到 Ready 的转换点，计算 `Draft Duration` 与 `Wait Time to Review`。
- **协同质量深度评估 (ELOC/Review Quality) [NEW]**:
  - **有效评论分析**: 追踪引发代码变更的评论比例 (`effective_comment_count`)，量化评审深度。
  - **秒批识别**: 建立 `rubber_stamp` 判定模型，基于评审时长与互动频率预警低质量评审。
- **安全漏洞采集 (DevSecOps) [NEW]**:
  - **全量漏洞同步**: 采集 SAST/DAST/Secret Detection 扫描结果。
  - **Findings 降级采集**: 支持从 Finding 级别回溯 CVE 漏洞，打通安全到研发的最后里程。
- **CALMS 深度扫描**: 追踪 Issue 的 `State/Label/Milestone` 原子事件，量化“等待浪费”。
