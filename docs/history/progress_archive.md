...
## 归档批次：2026-04-24 (会话 3)

### 1. [Hardening] 凭据隔离与二次确认协议落地: 固化了“凭据-意图防火墙”，确保敏感操作物理级授权。(2026-04-24)

### 2. [Spike-Result] 全链路极速性能探针: 落地 `tmpfs` 内存数据库与精准测试，增量测试耗时降至 \<5秒。(2026-04-24)

### 3. [Ad-hoc] 宿主机环境声明与双 CI 策略: 更新全局规范，明确 Windows 环境 PowerShell 限制，并在确立 Dual CI 防线。(2026-04-24)

## 归档批次：2026-04-29 (自动归档)

### 1. [Ad-hoc] 配置补齐与文档升级: 补全 `.env.example` 缺失项 (ZenTao, Nexus, E2E) 防止 CI 运行时崩溃，并升级 README 反映 Python 3.12+ 与 uv 架构。(2026-04-24)

### 2. [Cov] 架构核心层 90%+ 覆盖率: 补齐了 BaseWorker (94%) 与 Registry (99%) 的深度单元测试，确保了插件加载与数据持久化逻辑的健壮性。(2026-04-24)

______________________________________________________________________

## 归档批次：2026-04-24 (会话 2)

### 1. [Hardening] 预检脚本 Windows 适配: 修复了 pre-commit 钩子在 Windows 环境下的编码冲突，并解决了 Nexus/Scheduler 单元测试在目录重构后的回归故障，实现了全门禁本地 100% 跑通。(2026-04-24)

### 2. [Refactor] 测试目录规范化: 重构 tests/unit 目录，将 devops_collector 重命名为 test_collector，解决了 pytest 运行时的包阴影 (Shadowing) 冲突问题。(2026-04-24)

### 3. [Cov] 核心覆盖率攻坚: 实现了 core/services, core/algorithms, core/utils 等核心模块的单元测试，services 模块覆盖率达到 100%，整体核心逻辑稳步迈向 80% 目标。(2026-04-24)

### 4. [Fix] SCD Type 2 架构对齐: 修复 Organization 模型中 org_code 的 UNIQUE 约束冲突，支持了多版本历史记录并行存储，消除了更新时的 IntegrityError。(2026-04-24)

### 5. [Hardening] 离线测试门禁: 实现 DEVOPS_SKIP_AUDIT 环境变量，支持在单元测试中自动关停 RabbitMQ/Audit 引擎，大幅提升本地极速反馈效率。(2026-04-24)
...
### 2. [Security] 漏洞清剿: 用 `uv` 升级黑名单依赖，清除 `safety` 爆出的 2 个 CVE 漏洞。(2026-04-23)

### 3. [Nexus] 插件集成闭环: 完成 Nexus 自动化扫描与 142 个组件 (818 资产) 入库，修复 Worker 中的 NotNullViolation 缺陷。(2026-04-20)

### 4. [Sonar] 资产入库: 完成 SonarQube 全量接入 (76 SUCCESS / 1 FAILED)，修复 Worker 事务 Bug。(2026-04-20)

### 5. [AGENTS] 规范补强: 固化「四大 AI 原生编码原则」，硬化交接日志物理先行律。(2026-04-20)

______________________________________________________________________

## 归档批次：2026-04-20

### 1. [Hardening] 测试隔离加固: 重构治理模块测试(isolated_client)，消除全量跑批中的跨文件依赖污染。(2026-04-15)

### 2. [Fix] 核心集成修复: 解决 bcrypt 与 PRAGMA 方言冲突，全局通过率达 99.6% (266/267)。(2026-04-15)
...
- **记录**: 详见 `docs/history/lessons-learned.log`，将历史教训转化为 `skill-creator` 可识别的逻辑守护。
