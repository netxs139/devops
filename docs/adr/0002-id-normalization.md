...
1. **Staging 转换**: 所有的 Staging 模型在输出 ID 时，若源端为数值，必须显式执行 `::text` 转换。
1. **桥接模型**: 跨系统 ID 的关联（如 GitLab ID 到 Master Project ID）必须且仅能通过 `int_project_resource_map` 进行路由，严禁在 FCT 层进行非标的硬链接。
