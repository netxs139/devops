...
1. **命名公约**: 队列名称统一为 `{source}_tasks`（如 `gitlab_tasks`, `zentao_tasks`）。
1. **动态路由**: `MessageQueue.publish_task()` 根据 Payload 中的 `source` 字段自动分发到对应队列。
1. **SSOT (单一事实来源)**: 队列名称由 `PluginRegistry` 动态生成，严禁在各模块硬编码。
1. **公平消费**: Worker 进程使用 `prefetch_count=1` 并同时监听所有已注册队列，确保各源获得公平的处理时间片。
