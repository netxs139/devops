...
1. 无法精准推送给特定干系人（如项目负责人、需求评审人、Bug经办人）
...
1. 触发质量门禁检查失败
1. 验证项目负责人收到SSE推送
1. 验证消息内容包含 `project_id` 元数据

### 测试用例2: 测试执行失败通知

1. 执行一个测试用例并标记为失败
1. 验证用例创建者收到SSE推送
1. 验证消息包含 `issue_iid` 元数据

### 测试用例3: 需求评审通知

1. 更新一个需求的评审状态
1. 验证需求创建者收到SSE推送（评审人除外）
1. 验证消息包含 `new_state` 信息
...
1. **减少噪音**: 避免全员广播造成的信息过载
1. **可追溯性**: 推送日志记录，方便排查问题
1. **用户体验**: 实时收到关注事项的状态变更
