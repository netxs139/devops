...
1. 在左侧边栏，选择 **Settings (设置)** -> **Webhooks**。
1. 点击 **Add new webhook**（或直接在页面底部的表单中填写）。
1. **URL**: 输入您的 Webhook 接收地址。
   - 示例: `http://<your-server-ip>:8000/webhook`
1. **Trigger (触发器)**:
   - 勾选 **Issues events** (议题事件)。
1. **SSL verification**:
   - 如果您的服务没有配置 HTTPS，请**取消勾选** "Enable SSL verification"。
1. **Secret Token**: (可选) 可以在代码中增加验证逻辑，当前原型暂未校验，可留空。
1. 点击 **Add webhook**。

## 3. 验证同步

1. 在 GitLab 项目中找一个标签为 `type::test` 的 Issue。
1. 修改其实施状态（如手动添加 `test-result::passed` 标签并指派给某人）。
1. 检查您运行 `devops_portal` 的终端。
   - 您应当能看到类似以下的日志输出：
   ```text
