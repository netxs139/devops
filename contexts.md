...
1. **编码前思考 (Ask > Guess)**:
   ...
   - **困惑刹车**: 遇到不清晰的任务说明或可能导致物理灾难的操作，必须立即指出并要求澄清。
   - **诊断豁免 (Diagnostic Exemption)**: [NEW] 为确保排障效率，只读诊断指令（如 `ls`, `git status`, `cat`）豁免于“咨询态代码锁定”，AI 可在设置 `SafeToAutoRun: true` 的前提下自主执行。

1. **简洁优先 (Less is More)**:

   - **对抗过度工程**: 不要添加要求之外的功能，严禁为一次性代码创建复杂的抽象。
   - **逻辑重写律**: 如果 200 行代码可以通过逻辑优化写成 50 行，必须重写它。
   - **资深审计视角**: 检验标准为：如果不符合“奥卡姆剃刀”原则或让资深工程师觉得过于复杂，则视为违规。

1. **精准修改 (Precision First)**:

   - **局部封锁**: 修改现有代码时，严禁“顺手”改进相邻的无关代码、注释或格式（除非接到清理命令）。只准碰必须碰的部分。
   - **孤儿清理律**: 必须删除因本次改动而变得无用的导入 (import)、变量或函数。
   - **追溯性**: 每一行物理修改都必须能直接追溯到用户的具体请求。
...
- **Dashboard Strategy**: 参见 [7.7 章节](#77-%E7%9C%8B%E6%9D%BF%E6%95%B0%E6%8D%AE%E8%AE%BF%E9%97%AE%E5%8D%8F%E8%AE%AE-dashboard-data-access-protocol-new)。
...
1. **集成测试依赖级联注入规范 (Dependency Seeding) [MANDATORY]**:
...
1. **多源身份对齐**：涉及 `IdentityManager` 的 OneID 逻辑，这与 Postgres 的唯一约束处理机制紧密相关。
1. **插件 Worker 逻辑**：涉及从外部 Mock API 拉取数据并批量入库（`bulk_save`），容器能验证真实的文件路径处理与网络超时表现。
...
### 11.4 技术调研与架构决策命名规范 (Spike & ADR Naming) [NEW]
...
为确保 DevOps 平台在百万级数据量下的生产韧性，Agent 在代码设计与评审阶段必须强制执行以下性能护栏：

1. **全表扫描阻断 (Full-Scan Blocking)**：
   - 对于主数据表（MDM Tables）中涉及 `is_current=True` 的高频查询，**必须**使用 **部分索引 (Partial Index)**。
   - 审计基准：若 Agent 发现查询谓词中包含固定过滤条件且对应的索引是全量 B-Tree，必须主动提出重构。
1. **批量处理强制令 (Batch Processing Mandate)**：
   - 任何预期处理量 >1000 条的操作（如同步、导出、清洗），禁止在循环中执行数据库 I/O。
   - **红线**：同步逻辑中的 N+1 查询被视为 L3 级架构违规，必须改用 `batch_upsert` 或 `in_` 查询。
1. **容量预警阈值 (Capacity Warning Thresholds)**：
   - **主数据单表 > 1M 行**：必须考虑物理分区（Partitioning）或读写分离策略。
   - **唯一性冲突预判**：在 SCD2 模型中发现字段级 `unique=True` 时，必须立即触发「架构纠偏」流程。
1. **性能取证授权 (Profiling Authorization)**：
   - 授权 Agent 在提议 L3/L4 级重构前，主动通过 `EXPLAIN ANALYZE` (数据库) 或 `py-spy` (代码) 进行物理取证。
   - 提议改进时，必须包含“基于物理现状的量化预期”。
...
- **Justfile 核心指令速查表 [NEW]**:
  | 命令 | 用途 | 适用场景 |
  | :--- | :--- | :--- |
  | `just deploy` | 本地容器全栈启动（前后台+依赖） | 日常测试与验证 |
  | `just build` / `just package` | 构建镜像 / 导出离线 Tar 包 | 交付准备 |
  | `just verify` | 终极护栏检查 (Lint + 覆盖率 80%+) | L2 任务交付前强制执行 |
  | `just test` / `just test-int` | 容器内单元测试 / 容器内集成测试 | 开发中 / PR 合并前 |
  | `just lint` / `just ruff-fix` | Ruff 代码自查 / 自动修复 | 随手执行 |
  | `just sync-all` | 强制同步所有插件数据 | 数据一致性排障 |
  | `just deploy-offline` | 离线环境加载 Tar 镜像并启动 | 生产环境发布 |
  | `just clean` | 清理孤儿卷、临时配置和缓存文件 | 任务完工离场前环境卫生 |
  | `just security-audit` | 源码机密、SAST 及依赖漏洞全量审计 | L3/L4 高危重构交付前强制执行 |

- **Schema 同步**: 修改模型后必须执行 `just docs` 更新数据字典 `DATA_DICTIONARY.md`。
...
- **合并前检查清单 (Pre-merge Checklist)**:
  ...
  | 2 | **代码质量 (Lint)** | `just lint` 通过，无格式问题或死代码 | 🔴 BLOCK |
  | 3 | **单元测试** | `pytest tests/unit/` 全部通过 (本地快速验证) | 🔴 BLOCK |
  | 4 | **容器内测试** | `just test` 在 Docker 内通过 (环境一致性保证) | 🔴 BLOCK |
  | 5 | **容器部署验证** | `just deploy` 通过，健康检查无异常 | 🔴 BLOCK |
...
1. **全局身份解耦 (OneID Cross-Reference)**：
...
1. **隔离暂存区 (Staging Area & Promotion)**：
...
1. **异步对齐与自愈 (Async Auto-Alignment)**：
...
- 在数据库层存在业务循环引用时，必须遵循 [5.3 章节](#53-%E5%A4%96%E9%94%AE%E5%BE%AA%E7%8E%AF%E4%B8%8E%E6%B5%8B%E8%AF%95%E9%9A%94%E7%A6%BB%E8%A7%84%E8%8C%83-foreign-key-cycles--testing-isolation-newmandatory) 定义的“两阶段对齐协议”。
- 所有的“负责人/所有者”对齐操作必须具备防御性：允许在第一阶段为空，由第二阶段异步填充。

## 20. 数据隐私与合规治理 (Data Privacy & Governance) [NEW]

### 20.1 数据脱敏 (PII Masking)
...
### 20.2 行级权限过滤 (Row-Level Security)
...
## 21. 数据生命周期与归档策略 (Data Lifecycle & Archiving) [NEW]
...
## 22. 全链路可观测性与告警体系 (Observability & Alerting) [NEW]

### 22.1 结构化埋点与轻量级监控

- **轻量级指标方案**: 鉴于目前不引入重量级 Prometheus 服务，系统必须通过**结构化业务日志 (Structured Logging)** 输出全局的关键健康度指标。重点在日志中以特定字典格式暴露：各数据源采集延迟、MQ 队列处理耗时及死信率。
- **演进路线 `[Planned]`**: 未来流量爬坡时，考虑暴露 `/metrics` 端点引入 Prometheus 抓取。当前阶段一律使用分析 ELK 或轻量化 Python 本地采集脚本解决。

### 22.2 主动熔断与异常呼救阀值

- **容灾熔断锁**: 若插件 Worker 对特定外部生态 (如目标 GitLab Node) 连续遭受 10 次连接耗尽 / `HTTP 5xx` ，平台必须自发进入冷却休眠阻绝无尽重试引发的宿主机算力过载。
- **呼救闭环网**: 在全局调度拦截口需要下放告警 Webhook。当产生 `P0/Blocker` 灾难（如：数据库直连下线、外挂账本同步故障）时，立即向上级协作空间（如企业微信或告警邮箱）实施外联突发告警推送，确保无死角的实时告警响应。

## 23. 开放网关与 Webhook 契约规范 (Open API Gateway & Webhooks) [NEW]

### 23.1 被动接收接口红线

- **接收器解耦 (Non-blocking Push)**: 针对被动接受的第三方打点呼叫（如：构建流水线回调触发、代码安全扫描器回调），API 系统首要义务为瞬时落单（写入至 RabbitMQ 或持久化事务队列等异步工具），必须力保将应答第三方用时遏制在 `200ms` 下方。

### 23.2 强鉴权机制与限流防线 (Rate Limiting)

- 向外开放通信权限的 `/webhook` 端点一律执行 Token 发放准则与 HMAC 验签 / Timestamp 阈值管控，以封堵重放窃数据攻击。
- **请求限速防洪机制**: 对于暴露的 API，要制定严密的流量阻逆门槛。目前可通过 Python 内存算法或轻量级 Token Bucket 中间件实施限流抛弃策略，防止底座系统的无差别 DDoS 重大系统冲毁。

## 24. 查询性能降级与原生加速策略 (Query Degradation & Native Caching) [NEW]

### 24.1 Dashboard 负载分离与结果物化
