...
1. **绝对复用原则 (Strict Reuse Principle)**:
   - **不造轮子**: 凡是能用成熟开源生态、企业基础组件库或既定技术栈（如 VueUse, Day.js, Vue Query 等）解决的需求，**严禁自行开发**或重复封装低效、功能重叠的工具代码。
   - **生态对齐**: 在创建任何新的工具函数 (utils) 或基础业务逻辑前，必须强制前置排查是否有可靠的标准化实现，做到“生态优先，拒绝闭门造车”。
